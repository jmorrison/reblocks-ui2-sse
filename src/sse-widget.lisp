(uiop:define-package #:reblocks-ui2-sse
  (:use #:cl)
  (:nicknames #:reblocks-ui2-sse/sse-widget)
...
(defvar *route-created* nil
  "This variable will be set to true when first Ui2-Sse widget will be initialized.")

(defclass ui2-sse-route (40ants-routes/route:route)
  ())

(defmethod initialize-instance :after ((self ui2-sse-route) &rest args)
  (clouseau:inspect (list :initialize-instance self))
  self)
...
(defclass websocket-route (40ants-routes/route:route)
  ())


(defmacro websocket ((path &key name))
  "This macro creates a route of WEBSOCKET-ROUTE class.
  ...
  `(let ((route (40ants-routes/defroutes:get (,path :name ,name :route-class websocket-route)
                  nil)))
     (values route)))
...
Automatically adds a prefix depending on current webapp and widget."
  ...
  (make-instance 'websocket-route
                 )

  ;; (let ((server-routes (object-routes reblocks:*server*)))
...
(reblocks/widget:defwidget ui2-sse-widget ()
  ())


(defmethod initialize-instance ((widget ui2-sse-widget) &rest initargs)
  (declare (ignorable initargs))
  (let* ((dom-id (reblocks/widgets/dom:dom-id widget))
	 (route `(40ants-routes/defroutes:get (,dom-id :name ,dom-id)
				 (format "get 2!!!~%"))))
    (clouseau:inspect (list
		       :dom-id+route
		       dom-id
		       route
		       reblocks/variables:*current-app*
		       (reblocks/app:app-routes reblocks/variables:*current-app*)
		       #+NIL (40ants-routes/breadcrumbs:get-breadcrumbs)
		       #+NIL (40ants-routes/breadcrumbs:breadcrumb-route)
		       #+NIL (40ants-routes/breadcrumbs:breadcrumb-path)
		       #+NIL (40ants-routes/breadcrumbs:breadcrumb-title)))
    (call-next-method))
...
(defmethod reblocks/dependencies:get-dependencies ((widget ui2-sse-widget))
...
(defmethod reblocks/widget:update ((widget ui2-sse-widget)
                                   &key
                                   inserted-after
                                   inserted-before
                                   removed)
    ...
    (*background*
     (with-collected-dependencies ()
       (let* ((rendered-widget (unless removed
         ...
         ;; (when collected-deps)
         (reblocks/routes/server::register-dependencies collected-deps)
...
(defmethod reblocks/widget:render :after ((widget ui2-sse-widget))
  (unless *background*
...
(defmethod reblocks/widget:render ((widget ui2-sse-widget))
  (clouseau:inspect (list widget :reblocks/widget#\:render (40ants-routes/breadcrumbs:get-breadcrumbs)))
  (reblocks/response:send-script
   '(progn
     #+NIL (alert "Hello 1 from the sent script")
     (setup-s-s-e)
     #+NIL (alert "Hello 2 from the sent script")))
  (with-html ()
    (:p "reblocks/widget:render")))

(defmethod reblocks/dependencies:get-dependencies :around ((widget ui2-sse-widget))
  (list*
   #+NIL
   (reblocks-parenscript:make-dependency
    (alert "Hello from ui2-sse-widget get-dependencies"))
   (reblocks-parenscript:make-dependency
    (defun setup-s-s-e ()
      #+NIL (alert "lambda within setup-s-s-e")
      (let* ((current-script (chain document current-script))
	     (parent-div (chain current-script parent-node))
	     (ul (chain document (create-element "ul")))
	     (es (ps:new (-event-source #+NIL "/events" #-NIL (ps:lisp (reblocks/widgets/dom:dom-id widget))))))
	#+NIL (setf (chain ul id) "stdout")
	#-NIL (chain ul (set-attribute "id" "stdout"))
	(chain parent-div (append ul))

	(setf (chain ul style css-text) "list-style-type:none;")

	(chain console (log "document: " document))
	(chain console (log "current-script: " current-script))
	(chain console (log "parent-div: " parent-div))
	(chain console (log "ul: " ul))
	(chain console (log "ul by id: " (chain document (get-element-by-id "stdout"))))
	(chain console (log "es: " es))

	(flet ((add-another-list-element (text)
		 (let ((li (chain document (create-element "li"))))
		   (setf (chain li inner-text) text)
		   (chain ul (append-child li)))))
	  (setf (chain es onmessage) (lambda (event) (chain console (log "Received:" (chain event data)))))
	  (chain es (add-event-listener "open" (lambda () (add-another-list-element "Server connect :)"))))
	  (chain es (add-event-listener "my-custom-event" (lambda (event) (add-another-list-element (chain event data)))))
	  (chain es (add-event-listener "error" (lambda () (add-another-list-element "Server unavailable :("))))))))
   (call-next-method)))
  ...
  (flet ((let-bindings (&rest args)
    ...
    (let* ((woo-package (find-package :woo))
      ...
      `(let* ,(let-bindings
               ...
               '(server reblocks/variables::*server*)
               (when woo-package
         ...
         (bt:make-thread (lambda ()
                           (let ,(let-bindings
                                  ...
                                  '(reblocks/variables::*server* server)
                                  ;; Hack
                                  (when woo-package
